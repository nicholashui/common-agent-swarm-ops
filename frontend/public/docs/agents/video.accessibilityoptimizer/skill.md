---
name: video-accessibilityoptimizer
description: Role harness for video.accessibilityoptimizer — WCAG 2.2 contrast, captions, audio description, color-blind safe
version: 1.0.0
agent_id: video.accessibilityoptimizer
---

# Skill — `video.accessibilityoptimizer`

## When to use
Load this skill when the host routes a task to `video.accessibilityoptimizer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.accessibilityoptimizer.v1` from `../prompts/`.
2. Load rubric `video.rubric.accessibilityoptimizer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.accessibilityoptimizer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.accessibilityoptimizer/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 15, 21, 26, 30, 31, 37, 38, 45, 59, 63, 70, 71, 87, 88, 93, 94).
...
- Agent: `video.accessibilityoptimizer`
<!-- RETHINK_100:end -->

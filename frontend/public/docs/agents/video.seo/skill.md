---
name: video-seo
description: Role harness for video.seo — Optimizes discoverability through titles, descriptions, metadata, and search intent
version: 1.0.0
agent_id: video.seo
---
...
## When to use
Load this skill when the host routes a task to `video.seo` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.seo.v1` from `../prompts/`.
2. Load rubric `video.rubric.seo.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.seo` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.seo/golden.json`

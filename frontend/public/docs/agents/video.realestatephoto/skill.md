---
name: video-realestatephoto
description: Role harness for video.realestatephoto — Wide interiors; Matterport scans
version: 1.0.0
agent_id: video.realestatephoto
---

# Skill — `video.realestatephoto`

## When to use
Load this skill when the host routes a task to `video.realestatephoto` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.realestatephoto.v1` from `../prompts/`.
2. Load rubric `video.rubric.realestatephoto.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.realestatephoto` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.realestatephoto/golden.json`

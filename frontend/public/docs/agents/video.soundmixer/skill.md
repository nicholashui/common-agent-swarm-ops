---
name: video-soundmixer
description: Role harness for video.soundmixer — Final mix; deliverables (5.1/Atmos)
version: 1.0.0
agent_id: video.soundmixer
---

# Skill — `video.soundmixer`

## When to use
Load this skill when the host routes a task to `video.soundmixer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.soundmixer.v1` from `../prompts/`.
2. Load rubric `video.rubric.soundmixer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.soundmixer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.soundmixer/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 5, 14, 15, 21, 26, 30, 31, 37, 38, 39, 59, 63, 79, 87, 88, 91, 93, 94).

---
name: video-voiceclone
description: Role harness for video.voiceclone — Voice cloning + lip-sync
version: 1.0.0
agent_id: video.voiceclone
---

# Skill — `video.voiceclone`

## When to use
Load this skill when the host routes a task to `video.voiceclone` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.voiceclone.v1` from `../prompts/`.
2. Load rubric `video.rubric.voiceclone.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.voiceclone` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.voiceclone/golden.json`

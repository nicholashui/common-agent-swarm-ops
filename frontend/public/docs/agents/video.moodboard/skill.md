---
name: video-moodboard
description: Role harness for video.moodboard — Reference boards: visual, sonic, tonal
version: 1.0.0
agent_id: video.moodboard
---

# Skill — `video.moodboard`

## When to use
Load this skill when the host routes a task to `video.moodboard` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.moodboard.v1` from `../prompts/`.
2. Load rubric `video.rubric.moodboard.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.moodboard` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.moodboard/golden.json`

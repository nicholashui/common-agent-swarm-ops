...
description: Role harness for video.continuity — Maintains continuity across character, prop, wardrobe, environment, and time-state
version: 1.0.0
agent_id: video.continuity
---

# Skill — `video.continuity`

## When to use
Load this skill when the host routes a task to `video.continuity` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.continuity.v1` from `../prompts/`.
2. Load rubric `video.rubric.continuity.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.continuity` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.continuity/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 14, 15, 21, 26, 30, 31, 35, 37, 38, 40, 43, 48, 49, 50, 59, 63, 77, 87, 88, 90…).

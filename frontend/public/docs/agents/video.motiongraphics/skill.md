---
name: video-motiongraphics
description: Role harness for video.motiongraphics — Kinetic typography, lower thirds, infographics
version: 1.0.0
agent_id: video.motiongraphics
---

# Skill — `video.motiongraphics`

## When to use
Load this skill when the host routes a task to `video.motiongraphics` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.motiongraphics.v1` from `../prompts/`.
2. Load rubric `video.rubric.motiongraphics.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.motiongraphics` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.motiongraphics/golden.json`

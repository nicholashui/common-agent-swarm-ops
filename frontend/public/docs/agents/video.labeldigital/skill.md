---
name: video-labeldigital
description: Role harness for video.labeldigital — Runs label-side digital rollout, metadata, and channel packaging
version: 1.0.0
agent_id: video.labeldigital
---

# Skill — `video.labeldigital`

## When to use
Load this skill when the host routes a task to `video.labeldigital` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.labeldigital.v1` from `../prompts/`.
2. Load rubric `video.rubric.labeldigital.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.labeldigital` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.labeldigital/golden.json`

---
name: video-medicalillustrator
description: Role harness for video.medicalillustrator — Anatomy & procedure visuals
version: 1.0.0
agent_id: video.medicalillustrator
---

# Skill — `video.medicalillustrator`

## When to use
Load this skill when the host routes a task to `video.medicalillustrator` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.medicalillustrator.v1` from `../prompts/`.
2. Load rubric `video.rubric.medicalillustrator.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.medicalillustrator` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.medicalillustrator/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 2, 15, 21, 26, 30, 31, 37, 38, 48, 59, 63, 81, 87, 88, 93, 94).
...
- Agent: `video.medicalillustrator`
<!-- RETHINK_100:end -->

---
name: video-narrativearc
description: Role harness for video.narrativearc — 3-act / Save-the-Cat / Hero's Journey structure
version: 1.0.0
agent_id: video.narrativearc
---

# Skill — `video.narrativearc`

## When to use
Load this skill when the host routes a task to `video.narrativearc` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.narrativearc.v1` from `../prompts/`.
2. Load rubric `video.rubric.narrativearc.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.narrativearc` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.narrativearc/golden.json`

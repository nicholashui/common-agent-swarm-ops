---
name: video-brand
description: Role harness for video.brand — Enforces brand voice, claims boundaries, and visual consistency
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.brand` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.brand.v1` from `../prompts/`.
2. Load rubric `video.rubric.brand.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.brand` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.brand/golden.json`

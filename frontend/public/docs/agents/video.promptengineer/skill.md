---
name: video-promptengineer
description: Role harness for video.promptengineer — Crafts prompts; steers Sora/Veo/Runway/Kling
version: 1.0.0
agent_id: video.promptengineer
---

# Skill — `video.promptengineer`

## When to use
Load this skill when the host routes a task to `video.promptengineer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.promptengineer.v1` from `../prompts/`.
2. Load rubric `video.rubric.promptengineer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.promptengineer` via host agent runner
...
## Bindings
Shared pack special_skills (optional): llm_usage, optimization_agent
...
## Tests
- Offline golden: `business/video/evals/agents/video.promptengineer/golden.json`

---
name: video-mpa
description: Role harness for video.mpa — Prepares rating-related packaging and release-readiness inputs for feature workflows
version: 1.0.0
agent_id: video.mpa
---
...
## When to use
Load this skill when the host routes a task to `video.mpa` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.mpa.v1` from `../prompts/`.
2. Load rubric `video.rubric.mpa.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.mpa` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.mpa/golden.json`

...
description: Role harness for video.trendintelligence — Detects emerging memes, sounds, formats
version: 1.0.0
agent_id: video.trendintelligence
---

# Skill — `video.trendintelligence`

## When to use
Load this skill when the host routes a task to `video.trendintelligence` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.trendintelligence.v1` from `../prompts/`.
2. Load rubric `video.rubric.trendintelligence.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.trendintelligence` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.trendintelligence/golden.json`

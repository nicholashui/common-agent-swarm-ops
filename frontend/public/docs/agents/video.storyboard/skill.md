...
description: Role harness for video.storyboard — Script → shot panels
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.storyboard` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.storyboard.v1` from `../prompts/`.
2. Load rubric `video.rubric.storyboard.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.storyboard` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.storyboard/golden.json`

---
name: video-travelcine
description: Role harness for video.travelcine — Destination cinematography
version: 1.0.0
agent_id: video.travelcine
---

# Skill — `video.travelcine`

## When to use
Load this skill when the host routes a task to `video.travelcine` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.travelcine.v1` from `../prompts/`.
2. Load rubric `video.rubric.travelcine.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.travelcine` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.travelcine/golden.json`

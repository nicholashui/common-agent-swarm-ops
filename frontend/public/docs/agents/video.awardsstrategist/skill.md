---
name: video-awardsstrategist
description: Role harness for video.awardsstrategist — Plans awards submissions and campaign timing
version: 1.0.0
agent_id: video.awardsstrategist
---

# Skill — `video.awardsstrategist`

## When to use
Load this skill when the host routes a task to `video.awardsstrategist` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.awardsstrategist.v1` from `../prompts/`.
2. Load rubric `video.rubric.awardsstrategist.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.awardsstrategist` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.awardsstrategist/golden.json`

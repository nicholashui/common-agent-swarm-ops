---
name: video-cameraoperator
description: Role harness for video.cameraoperator — Executes framing / focus / move per DoP intent
version: 1.0.0
agent_id: video.cameraoperator
---

# Skill — `video.cameraoperator`

## When to use
Load this skill when the host routes a task to `video.cameraoperator` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.cameraoperator.v1` from `../prompts/`.
2. Load rubric `video.rubric.cameraoperator.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.cameraoperator` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.cameraoperator/golden.json`

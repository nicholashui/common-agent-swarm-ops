...
description: Role harness for video.orchestrator — Runs CrewAI/AutoGen/LangGraph DAG; retries, timeouts, fan-out/fan-in
version: 1.0.0
agent_id: video.orchestrator
---

# Skill — `video.orchestrator`

## When to use
Load this skill when the host routes a task to `video.orchestrator` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.orchestrator.v1` from `../prompts/`.
2. Load rubric `video.rubric.orchestrator.v1` from `../rubrics/`.
3. Ground only on `../sources/` and host memory namespaces.
4. Execute architecture patterns: Agentic Graph, Agent Skills.
...
- **Entry:** pack agent `video.orchestrator` via host agent runner
...
## Bindings
Shared pack special_skills (optional): agent_loop_v3, complex_problem_solution_process_model
...
## Tests
- Offline golden: `business/video/evals/agents/video.orchestrator/golden.json`

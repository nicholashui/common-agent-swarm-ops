---
name: video-analyst
description: Role harness for video.analyst — Aggregates business, creative, and technical performance telemetry into decision-ready reports
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.analyst` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.analyst.v1` from `../prompts/`.
2. Load rubric `video.rubric.analyst.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.analyst` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.analyst/golden.json`

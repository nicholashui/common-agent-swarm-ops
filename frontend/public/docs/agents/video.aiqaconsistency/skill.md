---
name: video-aiqaconsistency
description: Role harness for video.aiqaconsistency — Catches frame drift, hand/face artifacts, identity breaks
version: 1.0.0
agent_id: video.aiqaconsistency
---

# Skill — `video.aiqaconsistency`

## When to use
Load this skill when the host routes a task to `video.aiqaconsistency` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.aiqaconsistency.v1` from `../prompts/`.
2. Load rubric `video.rubric.aiqaconsistency.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.aiqaconsistency` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.aiqaconsistency/golden.json`

---
name: video-archiveresearch
description: Role harness for video.archiveresearch — Historical / academic / archival deep search
version: 1.0.0
agent_id: video.archiveresearch
---

# Skill — `video.archiveresearch`

## When to use
Load this skill when the host routes a task to `video.archiveresearch` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.archiveresearch.v1` from `../prompts/`.
2. Load rubric `video.rubric.archiveresearch.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.archiveresearch` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.archiveresearch/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 15, 21, 26, 30, 31, 37, 38, 59, 63, 69, 78, 87, 88, 93, 94).

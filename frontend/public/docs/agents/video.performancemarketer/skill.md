---
name: video-performancemarketer
description: Role harness for video.performancemarketer — Optimize ads for ROAS
version: 1.0.0
agent_id: video.performancemarketer
---

# Skill — `video.performancemarketer`

## When to use
Load this skill when the host routes a task to `video.performancemarketer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.performancemarketer.v1` from `../prompts/`.
2. Load rubric `video.rubric.performancemarketer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.performancemarketer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.performancemarketer/golden.json`
...
- Agent: `video.performancemarketer`
<!-- RETHINK_100:end -->

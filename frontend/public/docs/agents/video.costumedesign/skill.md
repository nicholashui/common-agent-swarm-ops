---
name: video-costumedesign
description: Role harness for video.costumedesign — Character-through-wardrobe
version: 1.0.0
agent_id: video.costumedesign
---

# Skill — `video.costumedesign`

## When to use
Load this skill when the host routes a task to `video.costumedesign` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.costumedesign.v1` from `../prompts/`.
2. Load rubric `video.rubric.costumedesign.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.costumedesign` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.costumedesign/golden.json`

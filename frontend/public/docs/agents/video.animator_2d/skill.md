---
name: video-animator_2d
description: Role harness for video.animator_2d — Character motion, weight, timing
version: 1.0.0
agent_id: video.animator_2d
---

# Skill — `video.animator_2d`

## When to use
Load this skill when the host routes a task to `video.animator_2d` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.animator_2d.v1` from `../prompts/`.
2. Load rubric `video.rubric.animator_2d.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.animator_2d` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.animator_2d/golden.json`

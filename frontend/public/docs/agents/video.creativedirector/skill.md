...
description: Role harness for video.creativedirector — Campaign concept; cross-discipline taste
version: 1.0.0
agent_id: video.creativedirector
---

# Skill — `video.creativedirector`

## When to use
Load this skill when the host routes a task to `video.creativedirector` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.creativedirector.v1` from `../prompts/`.
2. Load rubric `video.rubric.creativedirector.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.creativedirector` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.creativedirector/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 1, 2, 7, 11, 12, 15, 16, 21, 26, 30, 31, 33, 35, 37, 38, 43, 47, 48, 59, 63…).

---
name: video-archivemaster
description: Role harness for video.archivemaster — Produces archive-grade masters and preservation packages
version: 1.0.0
agent_id: video.archivemaster
---

# Skill — `video.archivemaster`

## When to use
Load this skill when the host routes a task to `video.archivemaster` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.archivemaster.v1` from `../prompts/`.
2. Load rubric `video.rubric.archivemaster.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.archivemaster` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.archivemaster/golden.json`

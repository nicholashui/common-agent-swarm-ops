---
name: video-instructionaldesign
description: Role harness for video.instructionaldesign — Learning objectives → script → assessment
version: 1.0.0
agent_id: video.instructionaldesign
---

# Skill — `video.instructionaldesign`

## When to use
Load this skill when the host routes a task to `video.instructionaldesign` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.instructionaldesign.v1` from `../prompts/`.
2. Load rubric `video.rubric.instructionaldesign.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.instructionaldesign` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.instructionaldesign/golden.json`
...
- Agent: `video.instructionaldesign`
<!-- RETHINK_100:end -->

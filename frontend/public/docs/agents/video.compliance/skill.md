...
description: Role harness for video.compliance — FTC, HIPAA, GDPR, IP, AI-likeness clearance
version: 1.0.0
agent_id: video.compliance
---

# Skill — `video.compliance`

## When to use
Load this skill when the host routes a task to `video.compliance` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.compliance.v1` from `../prompts/`.
2. Load rubric `video.rubric.compliance.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.compliance` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.compliance/golden.json`

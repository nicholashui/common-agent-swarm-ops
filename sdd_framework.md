# Common Agent Swarm Operation System CASOPS: SDD Governance, Evidence, and Conformance Framework
...
>
> CASOPS should make specifications portable, traceable, safely consumable, and testably interoperable across existing tools. It should be considered successful if its interchange model and conformance suite are adopted directly by existing ecosystems, even if the standalone CASOPS product eventually becomes unnecessary.

---

## 0. What Version 3.0 Changes

Version 2.0 correctly repositioned CASOPS from a replacement framework to a federation layer. Version 3.0 strengthens that direction and fixes several remaining architectural, scientific, security, and delivery problems.

| v2.0 issue | v3.0 correction |
|---|---|
| The canonical model, Markdown syntax, persistence format, and runtime API were conflated | Separates the **CASOPS Semantic Model**, **CASOPS Package Format**, authoring projections, adapters, context compiler, and runtime surfaces |
...
### 1.1 The problem CASOPS should solve
...
CASOPS will provide six public goods:
...
> **Keep authoring where you already author. Use CASOPS to exchange, inspect, verify, compile, and audit.**

### 1.3 What CASOPS is not
...
### 2.1 Source and platform policy
...
Each retained reference must record:
...
classification: non-configurational-reference
```

#### Supported-output boundary — mandatory

CASOPS supports configuration, integration outputs, and skill collection **only for Kiro and Claude Code**. It may emit only Kiro-native artifacts or Claude Code-native artifacts, including their approved instruction, skill, hook, steering, MCP, and static-context surfaces.

**Gemini and Gemini CLI are absolutely excluded.** CASOPS must never collect, download, ingest, curate, map, synchronize, or generate skills or configuration from Gemini or Gemini CLI. Discovery, adapters, importers, synchronizers, caches, registries, fixtures, and generated-output paths must reject Gemini and Gemini CLI material before persistence. No waiver, compatibility profile, plug-in, or future roadmap item may override this prohibition.

Material from any other platform may be retained only when needed as a non-executable, non-configurational reference with provenance and trust labels. CASOPS must not collect skills from it, generate configuration for it, map it to a target configuration, or synchronize it with Kiro or Claude Code configuration.
...
| Ecosystem | Capabilities CASOPS should preserve | What CASOPS must not duplicate |
|---|---|---|
...
Spec Kit currently documents more than 30 agent integrations, multi-install support, extensions, presets, bundles, and resumable workflows. CASOPS therefore should consume and test these artifacts rather than recreate their distribution machinery. ([github.github.com](https://github.github.com/spec-kit/index.html?utm_source=openai))

OpenSpec’s current model includes delta specifications, customizable artifact schemas, and beta coordination workspaces with context stores and multi-repository links. Cross-repository coordination is therefore not an untouched greenfield feature; CASOPS must interoperate with it rather than position a registry as a replacement. ([github.com](https://github.com/Fission-AI/OpenSpec/blob/main/docs/concepts.md?utm_source=openai))
...
### 2.3 Supported agent integration surfaces

CASOPS has exactly two supported output targets. Capability research about other agents is not an integration commitment and must remain non-configurational reference material under Section 2.1.

| Supported target | Confirmed integration surfaces | CASOPS approach |
|---|---|---|
...
| **Kiro** | Native specs, steering, hooks, MCP, custom agents, CLI Spec agent | Kiro artifact adapter plus native skill, steering, hook, and MCP integration |

Claude Code explicitly separates persistent instructions, on-demand skills, external MCP tools, deterministic hooks, and isolated subagents. That separation should inform CASOPS’s emitter and context architecture. ([code.claude.com](https://code.claude.com/docs/en/memory?utm_source=openai))

Kiro supports requirements-first and design-first feature specifications, bugfix specifications, Quick Plan, parallel task waves, and specification continuity across IDE, CLI, and web. Its CLI documentation describes a unified agent harness and a built-in Spec agent. ([kiro.dev](https://kiro.dev/docs/specs/quick-plan/?utm_source=openai))
...
| Research input | Evidence status | CASOPS consequence |
|---|---|---|
...
The interoperability survey supports using existing protocols for their intended roles rather than embedding CASOPS semantics into a new transport. ([arxiv.org](https://arxiv.org/abs/2505.02279))
...
2. **Kiro and Claude Code conformance**
   - The Kiro and Claude Code integrations pass their declared profiles; no other agent receives a configuration or skill output.
...
4. **Safe context compilation**
   - Context bundles are deterministic, budgeted, provenance-aware, and labeled by trust level for Kiro or Claude Code.
...
- emit configuration, integrations, or skills for any platform other than Kiro and Claude Code;
- collect, download, ingest, curate, map, synchronize, or generate skills or configuration from Gemini or Gemini CLI.
...
|---|---|
| Semantic model | CSM 1.0 with a deliberately small mandatory core |
| Package format | CPF directory and archive representation |
| Core CLI | Inspect, import, export, validate, diff, trace, compile context, record evidence, run gates |
| Adapters | Kiro artifact adapter; reference readers for Spec Kit, OpenSpec, and BMAD that retain only non-configurational source material |
| Agent integrations | Claude Code and Kiro only |
| Generated outputs | Claude Code `CLAUDE.md`/skills/hooks/MCP and Kiro steering/skills/hooks/MCP only |
...
| Security | Sandboxed adapters, trust labels, path protections, content-free telemetry, signed releases, and Gemini/Gemini CLI rejection |
| Conformance | Adapter, Kiro, Claude Code, security, and behavioral profiles |
| Documentation | Tool migration, Kiro and Claude Code integration, security, enterprise, and conformance guides |
...
- Additional non-configurational reference readers, subject to the source policy.
...
CASOPS must not use “100% compatible” without a profile.

A valid claim is:

> “Adapter X passes 100% of CASOPS Adapter Profile A3 for Tool Version Y, with zero unreported loss on Conformance Suite Version Z.”
...
| **A3 — Core Semantic Interop** | Supported CSM semantics survive import, export, and cross-tool projection | 100% deterministic invariants; zero unreported loss |
...
These profiles apply only to Kiro and Claude Code. They do not authorize output, skill collection, configuration mapping, or synchronization for any other platform.
...
```text
┌─────────────────────────────────────────────────────────────────┐
│ Kiro artifacts and approved non-configurational references      │
│ (reference material is never emitted as target configuration)   │
└───────────────────────┬─────────────────────────────────────────┘
                        │ versioned readers and Kiro adapter
                        ▼
┌─────────────────────────────────────────────────────────────────┐
│ CSM: typed semantic graph                                       │
│ requirements · constraints · decisions · tasks · evidence       │
│ approvals · waivers · source maps · provenance                  │
└───────────────┬────────────────┬──────────────────┬──────────────┘
                │                │                  │
                ▼                ▼                  ▼
       CPF interchange     Trace/evidence     Validation/policy
           packages            ledger              engine
               ...
               ┌─────────────────┼─────────────────┐
               ▼                 ▼                 ▼
       Claude Code MCP      Kiro native       Approved static
        skills/hooks         skills/hooks      context bundles
               │                 │                 │
               └─────────────────┼─────────────────┘
                                 ▼
...
### 7.5 Canonical representation versus output projections

The CSM is an abstract semantic model, not a Markdown template.
...
2. **Human-readable projection**
   ...
   - Intended for review and optional CASOPS-native authoring.
   - Not the only valid source syntax.

3. **Supported native output projections**
   - Kiro artifacts and Claude Code configuration only.

4. **Non-configurational reference preservation**
   - Approved third-party source material may be retained as a non-executable reference attachment with provenance and trust labels. It cannot become a configuration target, skill source, or synchronization endpoint.

This prevents a reference source from being mistaken for a supported configuration surface.

### 7.6 Package format

Illustrative CPF layout:
...
| `casops-authoritative` | CSM or CASOPS projection | Teams deliberately choosing CASOPS as source |
...
generator: "casops-adapter-openspec@0.4.0"
```

CASOPS will not support ungoverned dual-master editing.

### 7.8 Stable identity

Imported tools often do not provide stable identifiers. CASOPS therefore uses:

1. Native IDs where present.
2. Sidecar identity mappings persisted in `.casops/identities.json`.
...
{
  "operation": "replace",
  "target": "urn:casops:shop:req:cart-discount",
...
{
  "id": "evi-test-4f92",
  "subject": "urn:casops:shop:req:cart-discount",
  "kind": "test-run",
...
CASOPS reports multiple metrics rather than one gameable percentage:
...
|---|---|---|
| **Representation drift** | Native artifact no longer maps to recorded CSM revision | Block generated export; require reconciliation |
...
```yaml
bundle: ctx-8c91
task: urn:casops:shop:task:implement-cart-discount
target:
  agent: claude-code
...
budget:
  ...
  reserved:
    ...
    evidence: 4000
sources:
  - id: urn:casops:shop:req:cart-discount
    trust: approved
...
`project_T(S)` is the subset representable by tool `T`, not the complete CSM package.
...
MCP is exposed only through Kiro and Claude Code deployments. It is not a generic integration endpoint or a basis for configuration generation for another platform.
...
casops://workspace
casops://changes/{id}
casops://requirements/{id}
casops://tasks/{id}
casops://evidence/{id}
casops://context/{bundle-id}
```
...
#### Claude Code and Kiro skills

Ship and collect skills only for Claude Code and Kiro:
...
Skills from every other platform are out of scope. Gemini and Gemini CLI skills are prohibited and must be rejected under Section 2.1.

#### Instruction emitters

Generate the smallest appropriate project guidance only for:

- `CLAUDE.md` and Claude Code path-scoped instructions;
- Kiro steering and Kiro-native instruction surfaces.

Generated blocks must be bounded by markers and preserve user-authored content. CASOPS must not emit generic instruction files or target another platform.
...
#### External transport research

ACP or A2A material may be evaluated only as non-configurational reference material. CASOPS must not ship bindings, configurations, or skills for platforms beyond Kiro and Claude Code, and the core task, evidence, and governance state model must not depend on either protocol.
...
5. **Compile context**
   - Select Kiro or Claude Code as the target and apply its budget.
   - Generate a signed or hashed context bundle.

6. **Delegate**
   - Invoke only through the selected Kiro or Claude Code native skill, MCP, CLI, or static bundle.
...
CASOPS v1 supports coordination primitives, not autonomous negotiation:
...
### 9.1 Security posture and source exclusion

Specifications, adapters, plugins, MCP servers, imported repositories, generated context, tests, and evidence are all potentially untrusted. A model instruction is not a security boundary.

The Section 2.1 Gemini and Gemini CLI prohibition is a mandatory security and source-policy control. Detection must reject it before download, collection, ingestion, curation, mapping, synchronization, caching, fixture creation, or output generation. This control is non-waivable and applies to direct, transitive, and plug-in-mediated inputs.
...
| Prohibited Gemini or Gemini CLI material entering the system | Pre-persistence rejection, blocked download and discovery paths, denial audit event, and non-waivable policy enforcement |
...
No sanitizer can guarantee that a stochastic agent will ignore every malicious instruction. CASOPS therefore uses defense in depth:
...
- compare native and CASOPS-rendered inputs;
...
### 11.4 Kiro and Claude Code compatibility matrix

Only Kiro and Claude Code are tested as configuration or integration targets. A reference-material reader is not an agent integration and cannot be used to expand this matrix.

Test cadence:

| Cadence | Tests |
|---|---|
| Every commit | Deterministic core, adapter fixture, Kiro, and Claude Code configuration tests |
| Nightly | Kiro and Claude Code smoke tests on pinned configurations |
| Weekly | Expanded Kiro and Claude Code model/runtime matrix |
| Release candidate | Full declared conformance and security suite |
| After provider change | Targeted Kiro or Claude Code runtime-drift canary |
| Quarterly | Full behavioral benchmark refresh for the two supported targets |
...
| **Q3** | Kiro adapter, Kiro and Claude Code packs, trace/evidence, SpecOps, and internal beta |
...
| 4 | 7–8 | Remaining interviews; OpenSpec↔CSM↔Kiro spike; Phase 0 go/pivot/stop review |
| 5 | 9–10 | CSM v0.1; package manifest; canonicalization prototype; CLI `inspect` and `validate` |
...
| 11 | 21–22 | Context compiler v0.1; static bundle; Claude Code skill and Kiro steering emitters |
| 12 | 23–24 | Read-only MCP server; Claude Code and Kiro integrations |
...
| 16 | 31–32 | Kiro and Claude Code I1/I2 conformance packs; prohibited-source rejection tests |
...
- additional configuration or integration targets.
...
|---|---:|---|
| Backend engineer — semantic core | 1 | CSM, package format, canonicalization |
...
| Backend engineer — integrations | 1 | Kiro, Claude Code, MCP, CLI APIs |
...
- CSM normative specification;
- CPF normative specification;
- adapter contract;
...
- Kiro and Claude Code integration guides only;
...
|---|---|
| Source preservation | All four reference readers/adapters pass their declared A2 properties |
| Core interoperability | All four pass A3 for the mandatory CSM subset |
| Agent support | Kiro and Claude Code pass at least I2; no other agent has a configuration or skill output |
| Lifecycle integration | Kiro and Claude Code pass I3 where their approved surfaces permit it |
| Gemini exclusion | Rejection tests prove that Gemini and Gemini CLI cannot be collected, downloaded, ingested, curated, mapped, synchronized, or used to generate skills or configuration |
...
| Documentation | All committed migration, Kiro, Claude Code, and enterprise guides published |
...
then CASOPS has succeeded even if users no longer install a separate CASOPS CLI, provided all generated configurations and collected skills remain Kiro- and Claude Code-only.
...
|---|---|---|---|
| CASOPS becomes another competing format | Users begin authoring in CASOPS only because adapters are weak | Native-authoritative default; adoption measured by imports and conformance | Narrow to libraries and conformance |
...
| Eight-person scope overload | Milestones slip by more than one sprint | Scope freeze and stage gates | Defer UI and non-configurational reference readers |
...
- Complete OpenSpec↔CSM↔Kiro and Spec Kit↔CSM spikes.
- Measure semantic coverage and conversion loss.
- Draft CSM v0.1 and CPF v0.1.
...
- Release an experimental `casops inspect` and `casops validate`.
...
This is a projection of CSM content, not the canonical data model.

```markdown
---
casops_projection: "1.0"
package: "shop/cart"
...
casops inspect [path]
casops detect
casops import --from auto
casops export --to <tool>
casops validate
casops diff <base> <target>
casops loss-report
casops trace query <id>
casops trace coverage
casops evidence record
casops evidence verify
casops policy evaluate
casops gate check
casops context compile --task <id> --target <agent>
casops drift scan
casops package build
casops package verify
casops adapter list
casops adapter doctor
casops conformance run
casops serve --mcp --read-only
```
...
1. CASOPS is an SDD governance, evidence, and conformance framework.
...
9. MCP is an integration surface for Kiro and Claude Code, not the CASOPS protocol.
10. Claude Code and Kiro skills and instruction files are the only supported configuration distribution mechanisms.
11. Kiro and Claude Code support is capability-profiled and version-pinned; other material remains non-configurational reference only.
...
20. Gemini and Gemini CLI are permanently excluded from collection, ingestion, mapping, synchronization, skill collection, and configuration generation.
21. Failure of the Phase 0 hypotheses results in a narrower project or cancellation.

---

**End of `sdd_framework.md`**
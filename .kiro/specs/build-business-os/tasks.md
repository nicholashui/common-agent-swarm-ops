...
- [x] 2. Preserve and fully validate implemented Host foundations
...
- [x] 3. Complete scoped semantic retrieval and its target-local control-plane integration
  ...
  - [x] 3.2 Write Hypothesis **Property 7: Retrieval is semantic-first and cannot disclose across scope**.
...
- [x] 4. Validate the current versioned Host control plane and strict TypeScript operator shell
  ...
  - [x] 4.3 Add isolated API, frontend-contract, and LegacyEngine E1 evidence tests.
...
- [x] 5. Complete sandbox-only evolution validation
  ...
  - [x] 5.4 Add barrier-controlled evaluation/evolution integration tests.
...
- [x] 6. Deliver the remaining constrained, stub-only Video_Pack controls
  ...
  - [x] 6.2 Implement immutable video artifact versions, acyclic lineage validation, retained release requests, fail-closed release decisions, and only the necessary versioned video routes.
...
- [x] 7. Complete GraphEngine checkpoint recovery and dual-engine migration safety
  ...
  - [x] 7.2 Add isolated local Postgres integration tests for restart recovery, same-organization resume, pre-lookup cross-organization denial, and outage behavior.
    ...
    - _Requirements: 6.4_
  - [x] 7.3 Implement migration-gate assessment and atomic LegacyEngine retirement.
  ...
  - [x] 7.4 Write Hypothesis **Property 11: Migration gate satisfaction is an all-evidence conjunction**.
...
- [x] 8. Wire target-local Product-Bar evidence gates and final automated verification
  - [x] 8.1 Implement the deterministic evidence-gate runner.
  ...
  - [x] 8.2 Add final target-local evidence tests.

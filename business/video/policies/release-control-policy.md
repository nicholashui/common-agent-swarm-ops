...
## Critique control
Critiques must conform to `business/schemas/critique-message.schema.json`. A `blocker`, including one issued by `video.compliance`, prevents graph progress and release until a recorded resolution. A `major` finding permits no more than three refine/review attempts before `video.judge` or human escalation. `minor` and `nit` findings remain logged for learning and cannot override a failed release gate.

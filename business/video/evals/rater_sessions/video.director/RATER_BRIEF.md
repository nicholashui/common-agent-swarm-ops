...
- **unit:** `fraction`
- **threshold:** `agent_pairwise_win_rate >= 0.55`
- **pairwise min (if any):** 0.55
...
`business/video/evals/agents/video.director/golden.json`
...
```bash
python scripts/business/record_human_baseline.py --agent video.director --score <N> --rater <your_id> --notes "..."
```
...
```bash
python scripts/business/record_human_baseline.py --agent video.director --score 0 --rater <your_id> --evaluate
```

(Use session mode instead:)

```bash
python scripts/business/record_human_baseline.py --session --agent video.director --rater <your_id> --evaluate
```

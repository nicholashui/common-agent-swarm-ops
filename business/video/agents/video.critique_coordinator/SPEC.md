...
## Identity
- Common Agent ID: `video.critique_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain critique coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.critique_coordinator.v1`.
- Prompt reference: `video.prompt.critique_coordinator.v1`; references are inert local contract identifiers.
- Critique edges: `{"inputs":["video.judge_agent"],"outputs":["video.refine_coordinator"]}`.
...
```json
{"agent_id":"video.critique_coordinator","allowed_tools":[],"budget_policy":{"max_input_tokens":2048,"max_output_tokens":1024,"max_tool_requests":0},"critique_edges":{"inputs":["video.judge_agent"],"outputs":["video.refine_coordinator"]},"max_refinement_count":3,"model_policy":{"model_id":"local-video-config-v1","network_access":false,"provider":"local_deterministic"},"production_activation_requested":false,"prompt_reference":"video.prompt.critique_coordinator.v1","role":"Video Critique Coordinator configuration specialist","rubric_reference":"video.rubric.critique_coordinator.v1","schema_version":"1.0","status":"registered"}
```
...
- [Reviewed local source document](../../agents/video.critique_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.critic`.
- Relationship rationale: Human-reviewed mapping of video.critique_coordinator to generic source agent(s) video.critic for self-contained SPEC distillation. Distinct relationship for common role `video.critique_coordinator` (inventory order). Reviewed relationship unique to common agent `video.critique_coordinator`.

...
## Identity
- Common Agent ID: `video.continuity_supervisor`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain continuity supervisor outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.continuity)

Maintains continuity across character, prop, wardrobe, environment, and time-state

### Distilled quality (video.continuity)
...
- Local rubric reference: `video.rubric.continuity_supervisor.v1`.
- Prompt reference: `video.prompt.continuity_supervisor.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.continuity_supervisor/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.continuity`.
- Relationship rationale: Human-reviewed mapping of video.continuity_supervisor to generic source agent(s) video.continuity for self-contained SPEC distillation. Distinct relationship for common role `video.continuity_supervisor` (inventory order). Reviewed relationship unique to common agent `video.continuity_supervisor`.

...
## Identity
- Common Agent ID: `video.screenwriter`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain screenwriter outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.screenwriter.v1`.
- Prompt reference: `video.prompt.screenwriter.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.screenwriter/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.screenwriter`.
- Relationship rationale: Human-reviewed mapping of video.screenwriter to generic source agent(s) video.screenwriter for self-contained SPEC distillation. Distinct relationship for common role `video.screenwriter` (inventory order). Reviewed relationship unique to common agent `video.screenwriter`.

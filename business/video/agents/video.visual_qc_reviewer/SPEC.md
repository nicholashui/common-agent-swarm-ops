...
## Identity
- Common Agent ID: `video.visual_qc_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain visual qc reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
### Distilled responsibility (video.colorist)

Final grade; look consistency

### Distilled quality (video.colorist)
...
- Local rubric reference: `video.rubric.visual_qc_reviewer.v1`.
- Prompt reference: `video.prompt.visual_qc_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.visual_qc_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.colorist`, `video.critic`.
- Relationship rationale: Human-reviewed mapping of video.visual_qc_reviewer to generic source agent(s) video.critic, video.colorist for self-contained SPEC distillation. Distinct relationship for common role `video.visual_qc_reviewer` (inventory order). Reviewed relationship unique to common agent `video.visual_qc_reviewer`.

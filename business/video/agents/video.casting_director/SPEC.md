...
## Identity
- Common Agent ID: `video.casting_director`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain casting director outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.casting_director.v1`.
- Prompt reference: `video.prompt.casting_director.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.casting_director/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.casting`.
- Relationship rationale: Human-reviewed mapping of video.casting_director to generic source agent(s) video.casting for self-contained SPEC distillation. Distinct relationship for common role `video.casting_director` (inventory order). Reviewed relationship unique to common agent `video.casting_director`.

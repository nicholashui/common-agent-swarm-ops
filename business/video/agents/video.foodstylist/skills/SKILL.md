---
name: video-foodstylist
description: Role harness for video.foodstylist — Camera-ready food, recipe authenticity
version: 1.0.0
agent_id: video.foodstylist
---

# Skill — `video.foodstylist`

## When to use
Load this skill when the host routes a task to `video.foodstylist` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.foodstylist.v1` from `../prompts/`.
2. Load rubric `video.rubric.foodstylist.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.foodstylist` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.foodstylist/golden.json`

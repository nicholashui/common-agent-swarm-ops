...
## Identity
- Common Agent ID: `video.generative_media_operator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain generative media operator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.generative_media_operator.v1`.
- Prompt reference: `video.prompt.generative_media_operator.v1`; references are inert local contract identifiers.
...
```json
{"agent_id":"video.generative_media_operator","allowed_tools":["media.stub"],"budget_policy":{"max_input_tokens":2048,"max_output_tokens":1024,"max_tool_requests":0},"critique_edges":{"inputs":["video.critique_coordinator"],"outputs":["video.judge_agent"]},"max_refinement_count":3,"model_policy":{"model_id":"local-video-config-v1","network_access":false,"provider":"local_deterministic"},"production_activation_requested":false,"prompt_reference":"video.prompt.generative_media_operator.v1","role":"Video Generative Media Operator configuration specialist","rubric_reference":"video.rubric.generative_media_operator.v1","schema_version":"1.0","status":"registered"}
```
...
- [Reviewed local source document](../../agents/video.generative_media_operator/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.promptengineer`.
- Relationship rationale: Human-reviewed mapping of video.generative_media_operator to generic source agent(s) video.promptengineer for self-contained SPEC distillation. Distinct relationship for common role `video.generative_media_operator` (inventory order). Reviewed relationship unique to common agent `video.generative_media_operator`.

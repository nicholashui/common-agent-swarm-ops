...
## Identity
- Common Agent ID: `video.motion_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain motion designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.motion_designer.v1`.
- Prompt reference: `video.prompt.motion_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.motion_designer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.motiongraphics`.
- Relationship rationale: Human-reviewed mapping of video.motion_designer to generic source agent(s) video.motiongraphics for self-contained SPEC distillation. Distinct relationship for common role `video.motion_designer` (inventory order). Reviewed relationship unique to common agent `video.motion_designer`.

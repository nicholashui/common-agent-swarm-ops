...
## Identity
- Common Agent ID: `video.crm_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain crm coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.crm)

Delivers audience-targeted or trigger-based campaigns through CRM systems

### Distilled quality (video.crm)
...
- Local rubric reference: `video.rubric.crm_coordinator.v1`.
- Prompt reference: `video.prompt.crm_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.crm_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.crm`.
- Relationship rationale: Human-reviewed mapping of video.crm_coordinator to generic source agent(s) video.crm for self-contained SPEC distillation. Distinct relationship for common role `video.crm_coordinator` (inventory order). Reviewed relationship unique to common agent `video.crm_coordinator`.

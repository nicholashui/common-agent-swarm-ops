---
name: video-sounddesign
description: Role harness for video.sounddesign — Ambience, foley, SFX
version: 1.0.0
agent_id: video.sounddesign
---

# Skill — `video.sounddesign`

## When to use
Load this skill when the host routes a task to `video.sounddesign` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.sounddesign.v1` from `../prompts/`.
2. Load rubric `video.rubric.sounddesign.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.sounddesign` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.sounddesign/golden.json`

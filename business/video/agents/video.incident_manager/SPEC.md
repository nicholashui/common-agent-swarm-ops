...
## Identity
- Common Agent ID: `video.incident_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain incident manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.incident_manager.v1`.
- Prompt reference: `video.prompt.incident_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.incident_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.comms`, `video.gatekeeper`.
- Relationship rationale: Human-reviewed mapping of video.incident_manager to generic source agent(s) video.gatekeeper, video.comms for self-contained SPEC distillation. Distinct relationship for common role `video.incident_manager` (inventory order). Reviewed relationship unique to common agent `video.incident_manager`.

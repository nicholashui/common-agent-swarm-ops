...
description: Role harness for video.distributor — Manages downstream delivery to buyers, platforms, and territories
version: 1.0.0
agent_id: video.distributor
---

# Skill — `video.distributor`

## When to use
Load this skill when the host routes a task to `video.distributor` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.distributor.v1` from `../prompts/`.
2. Load rubric `video.rubric.distributor.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.distributor` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.distributor/golden.json`

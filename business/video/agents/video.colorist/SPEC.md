...
## Identity
- Common Agent ID: `video.colorist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain colorist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.colorist.v1`.
- Prompt reference: `video.prompt.colorist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.colorist/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.colorist`.
- Relationship rationale: Human-reviewed mapping of video.colorist to generic source agent(s) video.colorist for self-contained SPEC distillation. Distinct relationship for common role `video.colorist` (inventory order). Reviewed relationship unique to common agent `video.colorist`.

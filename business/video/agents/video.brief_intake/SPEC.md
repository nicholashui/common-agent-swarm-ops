...
## Identity
- Common Agent ID: `video.brief_intake`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain brief intake outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.brief_intake.v1`.
- Prompt reference: `video.prompt.brief_intake.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.brief_intake/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.planner`.
- Relationship rationale: Human-reviewed mapping of video.brief_intake to generic source agent(s) video.planner for self-contained SPEC distillation. Distinct relationship for common role `video.brief_intake` (inventory order). Reviewed relationship unique to common agent `video.brief_intake`.

...
## Identity
- Common Agent ID: `video.analytics_reporter`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain analytics reporter outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.analytics_reporter.v1`.
- Prompt reference: `video.prompt.analytics_reporter.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.analytics_reporter/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.analyst`.
- Relationship rationale: Human-reviewed mapping of video.analytics_reporter to generic source agent(s) video.analyst for self-contained SPEC distillation. Distinct relationship for common role `video.analytics_reporter` (inventory order). Reviewed relationship unique to common agent `video.analytics_reporter`.

...
## Identity
- Common Agent ID: `video.cinematography_planner`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain cinematography planner outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.cinematography_planner.v1`.
- Prompt reference: `video.prompt.cinematography_planner.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.cinematography_planner/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.cinematographer`.
- Relationship rationale: Human-reviewed mapping of video.cinematography_planner to generic source agent(s) video.cinematographer for self-contained SPEC distillation. Distinct relationship for common role `video.cinematography_planner` (inventory order). Reviewed relationship unique to common agent `video.cinematography_planner`.

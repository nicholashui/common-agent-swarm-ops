...
## Identity
- Common Agent ID: `video.vendor_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain vendor coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.vendor_coordinator.v1`.
- Prompt reference: `video.prompt.vendor_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.vendor_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.comms`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.vendor_coordinator to generic source agent(s) video.producer, video.comms for self-contained SPEC distillation. Distinct relationship for common role `video.vendor_coordinator` (inventory order). Reviewed relationship unique to common agent `video.vendor_coordinator`.

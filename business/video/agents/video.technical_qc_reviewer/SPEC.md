...
## Identity
- Common Agent ID: `video.technical_qc_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain technical qc reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.technical_qc_reviewer.v1`.
- Prompt reference: `video.prompt.technical_qc_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.technical_qc_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.critic`, `video.standardseditor`.
- Relationship rationale: Human-reviewed mapping of video.technical_qc_reviewer to generic source agent(s) video.standardseditor, video.critic for self-contained SPEC distillation. Distinct relationship for common role `video.technical_qc_reviewer` (inventory order). Reviewed relationship unique to common agent `video.technical_qc_reviewer`.

...
## Identity
- Common Agent ID: `video.release_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain release manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.release_manager.v1`.
- Prompt reference: `video.prompt.release_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.release_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.distributor`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.release_manager to generic source agent(s) video.distributor, video.producer for self-contained SPEC distillation. Distinct relationship for common role `video.release_manager` (inventory order). Reviewed relationship unique to common agent `video.release_manager`.

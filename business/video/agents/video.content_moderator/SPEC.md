...
## Identity
- Common Agent ID: `video.content_moderator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain content moderator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.content_moderator.v1`.
- Prompt reference: `video.prompt.content_moderator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.content_moderator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.gatekeeper`, `video.trustsafety`.
- Relationship rationale: Human-reviewed mapping of video.content_moderator to generic source agent(s) video.trustsafety, video.gatekeeper for self-contained SPEC distillation. Distinct relationship for common role `video.content_moderator` (inventory order). Reviewed relationship unique to common agent `video.content_moderator`.

---
name: video-localizationqa
description: Role harness for video.localizationqa — Translation + cultural fit
version: 1.0.0
agent_id: video.localizationqa
---

# Skill — `video.localizationqa`

## When to use
Load this skill when the host routes a task to `video.localizationqa` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.localizationqa.v1` from `../prompts/`.
2. Load rubric `video.rubric.localizationqa.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.localizationqa` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.localizationqa/golden.json`

...
## Identity
- Common Agent ID: `video.resource_planner`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain resource planner outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.resource_planner.v1`.
- Prompt reference: `video.prompt.resource_planner.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.resource_planner/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.planner`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.resource_planner to generic source agent(s) video.producer, video.planner for self-contained SPEC distillation. Distinct relationship for common role `video.resource_planner` (inventory order). Reviewed relationship unique to common agent `video.resource_planner`.

...
## Identity
- Common Agent ID: `video.signoff_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain signoff coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.signoff_coordinator.v1`.
- Prompt reference: `video.prompt.signoff_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.signoff_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.gatekeeper`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.signoff_coordinator to generic source agent(s) video.gatekeeper, video.producer for self-contained SPEC distillation. Distinct relationship for common role `video.signoff_coordinator` (inventory order). Reviewed relationship unique to common agent `video.signoff_coordinator`.

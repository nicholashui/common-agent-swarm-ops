...
## Identity
- Common Agent ID: `video.animation_director`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain animation director outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.animator_2d)
...
- Local rubric reference: `video.rubric.animation_director.v1`.
- Prompt reference: `video.prompt.animation_director.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.animation_director/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.animator_2d`.
- Relationship rationale: Human-reviewed mapping of video.animation_director to generic source agent(s) video.animator_2d for self-contained SPEC distillation. Distinct relationship for common role `video.animation_director` (inventory order). Reviewed relationship unique to common agent `video.animation_director`.

...
## Identity
- Common Agent ID: `video.postproduction_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain postproduction coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.postproduction_coordinator.v1`.
- Prompt reference: `video.prompt.postproduction_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.postproduction_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.editor`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.postproduction_coordinator to generic source agent(s) video.editor, video.producer for self-contained SPEC distillation. Distinct relationship for common role `video.postproduction_coordinator` (inventory order). Reviewed relationship unique to common agent `video.postproduction_coordinator`.

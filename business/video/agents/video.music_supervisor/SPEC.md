...
## Identity
- Common Agent ID: `video.music_supervisor`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain music supervisor outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.musicsupervisor)

Manages music fit, cue usage, rights awareness, and soundtrack packaging

### Distilled quality (video.musicsupervisor)
...
- Local rubric reference: `video.rubric.music_supervisor.v1`.
- Prompt reference: `video.prompt.music_supervisor.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.music_supervisor/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.musicsupervisor`.
- Relationship rationale: Human-reviewed mapping of video.music_supervisor to generic source agent(s) video.musicsupervisor for self-contained SPEC distillation. Distinct relationship for common role `video.music_supervisor` (inventory order). Reviewed relationship unique to common agent `video.music_supervisor`.

...
## Identity
- Common Agent ID: `video.client_success_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain client success manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.sales)

Handles buyer-facing sales packaging for distributors and outlets

### Distilled quality (video.sales)
...
- Local rubric reference: `video.rubric.client_success_manager.v1`.
- Prompt reference: `video.prompt.client_success_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.client_success_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.comms`, `video.sales`.
- Relationship rationale: Human-reviewed mapping of video.client_success_manager to generic source agent(s) video.sales, video.comms for self-contained SPEC distillation. Distinct relationship for common role `video.client_success_manager` (inventory order). Reviewed relationship unique to common agent `video.client_success_manager`.

...
## Identity
- Common Agent ID: `video.audit_liaison`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain audit liaison outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.audit_liaison.v1`.
- Prompt reference: `video.prompt.audit_liaison.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.audit_liaison/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.compliance`, `video.legal`.
- Relationship rationale: Human-reviewed mapping of video.audit_liaison to generic source agent(s) video.compliance, video.legal for self-contained SPEC distillation. Distinct relationship for common role `video.audit_liaison` (inventory order). Reviewed relationship unique to common agent `video.audit_liaison`.

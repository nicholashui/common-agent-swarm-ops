...
description: Role harness for video.marketing — Packages content for launch, promotions, and release sequencing
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.marketing` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.marketing.v1` from `../prompts/`.
2. Load rubric `video.rubric.marketing.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.marketing` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.marketing/golden.json`

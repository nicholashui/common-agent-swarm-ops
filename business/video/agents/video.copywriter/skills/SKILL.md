...
description: Role harness for video.copywriter — Scripts, captions, hooks, headlines
version: 1.0.0
agent_id: video.copywriter
---

# Skill — `video.copywriter`

## When to use
Load this skill when the host routes a task to `video.copywriter` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.copywriter.v1` from `../prompts/`.
2. Load rubric `video.rubric.copywriter.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.copywriter` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.copywriter/golden.json`

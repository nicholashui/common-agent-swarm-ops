...
## Identity
- Common Agent ID: `video.hook_specialist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain hook specialist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.copywriter)

Scripts, captions, hooks, headlines

### Distilled quality (video.copywriter)

Reading grade; hook-curiosity score; brand-voice cosine ≥0.85

### Distilled responsibility (video.ugccreator)

Authentic-feel ads in creator voice

### Distilled quality (video.ugccreator)
...
- Local rubric reference: `video.rubric.hook_specialist.v1`.
- Prompt reference: `video.prompt.hook_specialist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.hook_specialist/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.copywriter`, `video.ugccreator`.
- Relationship rationale: Human-reviewed mapping of video.hook_specialist to generic source agent(s) video.copywriter, video.ugccreator for self-contained SPEC distillation. Distinct relationship for common role `video.hook_specialist` (inventory order). Reviewed relationship unique to common agent `video.hook_specialist`.

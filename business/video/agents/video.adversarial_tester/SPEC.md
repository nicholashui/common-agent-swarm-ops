...
## Identity
- Common Agent ID: `video.adversarial_tester`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain adversarial tester outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.adversarial_tester.v1`.
- Prompt reference: `video.prompt.adversarial_tester.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.adversarial_tester/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.safetyredteam`.
- Relationship rationale: Human-reviewed mapping of video.adversarial_tester to generic source agent(s) video.safetyredteam for self-contained SPEC distillation. Distinct relationship for common role `video.adversarial_tester` (inventory order). Reviewed relationship unique to common agent `video.adversarial_tester`.

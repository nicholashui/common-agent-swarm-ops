...
## Identity
- Common Agent ID: `video.delivery_qc_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain delivery qc reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.distributor)

Manages downstream delivery to buyers, platforms, and territories

### Distilled quality (video.distributor)
...
- Local rubric reference: `video.rubric.delivery_qc_reviewer.v1`.
- Prompt reference: `video.prompt.delivery_qc_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.delivery_qc_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.distributor`, `video.gatekeeper`.
- Relationship rationale: Human-reviewed mapping of video.delivery_qc_reviewer to generic source agent(s) video.distributor, video.gatekeeper for self-contained SPEC distillation. Distinct relationship for common role `video.delivery_qc_reviewer` (inventory order). Reviewed relationship unique to common agent `video.delivery_qc_reviewer`.

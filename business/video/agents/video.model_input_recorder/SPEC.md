...
## Identity
- Common Agent ID: `video.model_input_recorder`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain model input recorder outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.promptengineer)

Crafts prompts; steers Sora/Veo/Runway/Kling

### Distilled quality (video.promptengineer)

Prompt→output CLIP-T; iteration count to acceptance; seed reproducibility

### Distilled responsibility (video.memory)

Episodic + long-term project memory; retrieval for any agent

### Distilled quality (video.memory)
...
- Local rubric reference: `video.rubric.model_input_recorder.v1`.
- Prompt reference: `video.prompt.model_input_recorder.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.model_input_recorder/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.memory`, `video.promptengineer`.
- Relationship rationale: Human-reviewed mapping of video.model_input_recorder to generic source agent(s) video.promptengineer, video.memory for self-contained SPEC distillation. Distinct relationship for common role `video.model_input_recorder` (inventory order). Reviewed relationship unique to common agent `video.model_input_recorder`.

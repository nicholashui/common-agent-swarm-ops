...
## Identity
- Common Agent ID: `video.aiqa_consistency`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain aiqa consistency outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.aiqaconsistency)

Catches frame drift, hand/face artifacts, identity breaks

### Distilled quality (video.aiqaconsistency)
...
- Local rubric reference: `video.rubric.aiqa_consistency.v1`.
- Prompt reference: `video.prompt.aiqa_consistency.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.aiqa_consistency/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.aiqaconsistency`.
- Relationship rationale: Human-reviewed mapping of video.aiqa_consistency to generic source agent(s) video.aiqaconsistency for self-contained SPEC distillation. Distinct relationship for common role `video.aiqa_consistency` (inventory order). Reviewed relationship unique to common agent `video.aiqa_consistency`.

...
## Identity
- Common Agent ID: `video.audio_qc_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain audio qc reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.audio_qc_reviewer.v1`.
- Prompt reference: `video.prompt.audio_qc_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.audio_qc_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.critic`, `video.soundmixer`.
- Relationship rationale: Human-reviewed mapping of video.audio_qc_reviewer to generic source agent(s) video.soundmixer, video.critic for self-contained SPEC distillation. Distinct relationship for common role `video.audio_qc_reviewer` (inventory order). Reviewed relationship unique to common agent `video.audio_qc_reviewer`.

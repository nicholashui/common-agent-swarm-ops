---
name: video-evaluationharness
description: Role harness for video.evaluationharness — Runs benchmarks (VBench, EvalCrafter, MT-Bench, FVD, CLIP-T); posts regressions
version: 1.0.0
agent_id: video.evaluationharness
---

# Skill — `video.evaluationharness`

## When to use
Load this skill when the host routes a task to `video.evaluationharness` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.evaluationharness.v1` from `../prompts/`.
2. Load rubric `video.rubric.evaluationharness.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.evaluationharness` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.evaluationharness/golden.json`

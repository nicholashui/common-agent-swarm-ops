...
## Identity
- Common Agent ID: `video.safety_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain safety reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.safetyredteam)

Adversarially attacks for deepfake, bias, jailbreak, defamation

### Distilled quality (video.safetyredteam)
...
- Local rubric reference: `video.rubric.safety_reviewer.v1`.
- Prompt reference: `video.prompt.safety_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.safety_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.safetyredteam`, `video.trustsafety`.
- Relationship rationale: Human-reviewed mapping of video.safety_reviewer to generic source agent(s) video.safetyredteam, video.trustsafety for self-contained SPEC distillation. Distinct relationship for common role `video.safety_reviewer` (inventory order). Reviewed relationship unique to common agent `video.safety_reviewer`.

...
## Identity
- Common Agent ID: `video.production_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain production designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.production_designer.v1`.
- Prompt reference: `video.prompt.production_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.production_designer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.productiondesign`.
- Relationship rationale: Human-reviewed mapping of video.production_designer to generic source agent(s) video.productiondesign for self-contained SPEC distillation. Distinct relationship for common role `video.production_designer` (inventory order). Reviewed relationship unique to common agent `video.production_designer`.

---
name: video-socialmediastrategist
description: Role harness for video.socialmediastrategist — Platform-native distribution, timing, trends
version: 1.0.0
agent_id: video.socialmediastrategist
---

# Skill — `video.socialmediastrategist`

## When to use
Load this skill when the host routes a task to `video.socialmediastrategist` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.socialmediastrategist.v1` from `../prompts/`.
2. Load rubric `video.rubric.socialmediastrategist.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.socialmediastrategist` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.socialmediastrategist/golden.json`
...
- Agent: `video.socialmediastrategist`
<!-- RETHINK_100:end -->

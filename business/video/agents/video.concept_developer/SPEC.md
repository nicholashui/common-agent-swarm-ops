...
## Identity
- Common Agent ID: `video.concept_developer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain concept developer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.ideation)

Divergent brainstorm of concepts, hooks, taglines

### Distilled quality (video.ideation)
...
- Local rubric reference: `video.rubric.concept_developer.v1`.
- Prompt reference: `video.prompt.concept_developer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.concept_developer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.conceptartist`, `video.ideation`.
- Relationship rationale: Human-reviewed mapping of video.concept_developer to generic source agent(s) video.ideation, video.conceptartist for self-contained SPEC distillation. Distinct relationship for common role `video.concept_developer` (inventory order). Reviewed relationship unique to common agent `video.concept_developer`.

...
## Identity
- Common Agent ID: `video.asset_librarian`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain asset librarian outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.asset_librarian.v1`.
- Prompt reference: `video.prompt.asset_librarian.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.asset_librarian/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.archivemaster`.
- Relationship rationale: Human-reviewed mapping of video.asset_librarian to generic source agent(s) video.archivemaster for self-contained SPEC distillation. Distinct relationship for common role `video.asset_librarian` (inventory order). Reviewed relationship unique to common agent `video.asset_librarian`.

...
## Identity
- Common Agent ID: `video.asset_security_officer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain asset security officer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.asset_security_officer.v1`.
- Prompt reference: `video.prompt.asset_security_officer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.asset_security_officer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.archivemaster`, `video.trustsafety`.
- Relationship rationale: Human-reviewed mapping of video.asset_security_officer to generic source agent(s) video.trustsafety, video.archivemaster for self-contained SPEC distillation. Distinct relationship for common role `video.asset_security_officer` (inventory order). Reviewed relationship unique to common agent `video.asset_security_officer`.

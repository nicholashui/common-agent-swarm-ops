...
## Identity
- Common Agent ID: `video.web_researcher`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain web researcher outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.webresearch)

Live web search, source ranking, citation extraction

### Distilled quality (video.webresearch)
...
- Local rubric reference: `video.rubric.web_researcher.v1`.
- Prompt reference: `video.prompt.web_researcher.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.web_researcher/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.webresearch`.
- Relationship rationale: Human-reviewed mapping of video.web_researcher to generic source agent(s) video.webresearch for self-contained SPEC distillation. Distinct relationship for common role `video.web_researcher` (inventory order). Reviewed relationship unique to common agent `video.web_researcher`.

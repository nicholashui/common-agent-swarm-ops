...
## Identity
- Common Agent ID: `video.social_editor`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain social editor outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.socialmediastrategist)
...
- Local rubric reference: `video.rubric.social_editor.v1`.
- Prompt reference: `video.prompt.social_editor.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.social_editor/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.editor`, `video.socialmediastrategist`.
- Relationship rationale: Human-reviewed mapping of video.social_editor to generic source agent(s) video.socialmediastrategist, video.editor for self-contained SPEC distillation. Distinct relationship for common role `video.social_editor` (inventory order). Reviewed relationship unique to common agent `video.social_editor`.

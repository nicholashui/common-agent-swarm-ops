...
## Identity
- Common Agent ID: `video.compliance_agent`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain compliance agent outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.compliance)

FTC, HIPAA, GDPR, IP, AI-likeness clearance

### Distilled quality (video.compliance)
...
- Local rubric reference: `video.rubric.compliance_agent.v1`.
- Prompt reference: `video.prompt.compliance_agent.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.compliance_agent/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.compliance`.
- Relationship rationale: Human-reviewed mapping of video.compliance_agent to generic source agent(s) video.compliance for self-contained SPEC distillation. Distinct relationship for common role `video.compliance_agent` (inventory order). Reviewed relationship unique to common agent `video.compliance_agent`.

...
## Identity
- Common Agent ID: `video.translation_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain translation reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.translation_reviewer.v1`.
- Prompt reference: `video.prompt.translation_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.translation_reviewer/agent_spec.json)
...
- Relationship rationale: Human-reviewed mapping of video.translation_reviewer to generic source agent(s) video.localizationqa for self-contained SPEC distillation. Distinct relationship for common role `video.translation_reviewer` (inventory order). Reviewed relationship unique to common agent `video.translation_reviewer`.

...
## Identity
- Common Agent ID: `video.edit_assembler`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain edit assembler outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.edit_assembler.v1`.
- Prompt reference: `video.prompt.edit_assembler.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.edit_assembler/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.editor`.
- Relationship rationale: Human-reviewed mapping of video.edit_assembler to generic source agent(s) video.editor for self-contained SPEC distillation. Distinct relationship for common role `video.edit_assembler` (inventory order). Reviewed relationship unique to common agent `video.edit_assembler`.

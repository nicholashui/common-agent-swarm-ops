...
## Identity
- Common Agent ID: `video.evaluation_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain evaluation designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.evaluation_designer.v1`.
- Prompt reference: `video.prompt.evaluation_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.evaluation_designer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.evaluationharness`.
- Relationship rationale: Human-reviewed mapping of video.evaluation_designer to generic source agent(s) video.evaluationharness for self-contained SPEC distillation. Distinct relationship for common role `video.evaluation_designer` (inventory order). Reviewed relationship unique to common agent `video.evaluation_designer`.

---
name: video-ux
description: Role harness for video.ux — Reviews clarity and usability of personalized or interactive outputs
version: 1.0.0
agent_id: video.ux
---
...
## When to use
Load this skill when the host routes a task to `video.ux` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.ux.v1` from `../prompts/`.
2. Load rubric `video.rubric.ux.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.ux` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.ux/golden.json`

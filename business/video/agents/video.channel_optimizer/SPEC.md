...
## Identity
- Common Agent ID: `video.channel_optimizer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain channel optimizer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.channelmanager)

Manages episodic or platform channel operations for cadence and metadata readiness

### Distilled quality (video.channelmanager)
...
- Local rubric reference: `video.rubric.channel_optimizer.v1`.
- Prompt reference: `video.prompt.channel_optimizer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.channel_optimizer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.channelmanager`.
- Relationship rationale: Human-reviewed mapping of video.channel_optimizer to generic source agent(s) video.channelmanager for self-contained SPEC distillation. Distinct relationship for common role `video.channel_optimizer` (inventory order). Reviewed relationship unique to common agent `video.channel_optimizer`.

...
## Identity
- Common Agent ID: `video.workflow_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain workflow designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.planner)

Decomposes brief into phased DAG with assignments + critic gates

### Distilled quality (video.planner)
...
- Local rubric reference: `video.rubric.workflow_designer.v1`.
- Prompt reference: `video.prompt.workflow_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.workflow_designer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.orchestrator`, `video.planner`.
- Relationship rationale: Human-reviewed mapping of video.workflow_designer to generic source agent(s) video.planner, video.orchestrator for self-contained SPEC distillation. Distinct relationship for common role `video.workflow_designer` (inventory order). Reviewed relationship unique to common agent `video.workflow_designer`.

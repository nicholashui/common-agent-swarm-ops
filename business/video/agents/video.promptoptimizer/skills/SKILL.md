---
name: video-promptoptimizer
description: Role harness for video.promptoptimizer — Auto-improves prompts via OPRO/APE/DSPy/Promptbreeder
version: 1.0.0
agent_id: video.promptoptimizer
---

# Skill — `video.promptoptimizer`

## When to use
Load this skill when the host routes a task to `video.promptoptimizer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.promptoptimizer.v1` from `../prompts/`.
2. Load rubric `video.rubric.promptoptimizer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.promptoptimizer` via host agent runner
...
## Bindings
Shared pack special_skills (optional): optimization_agent, llm_usage
...
## Tests
- Offline golden: `business/video/evals/agents/video.promptoptimizer/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 10, 11, 15, 17, 21, 26, 30, 31, 37, 38, 59, 63, 64, 87, 88, 93, 94).

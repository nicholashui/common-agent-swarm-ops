...
## Identity
- Common Agent ID: `video.audience_researcher`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain audience researcher outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.audiencesim)

Simulates audience preference, engagement, and drop-off

### Distilled quality (video.audiencesim)
...
- Local rubric reference: `video.rubric.audience_researcher.v1`.
- Prompt reference: `video.prompt.audience_researcher.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.audience_researcher/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.analyst`, `video.audiencesim`.
- Relationship rationale: Human-reviewed mapping of video.audience_researcher to generic source agent(s) video.audiencesim, video.analyst for self-contained SPEC distillation. Distinct relationship for common role `video.audience_researcher` (inventory order). Reviewed relationship unique to common agent `video.audience_researcher`.

...
## Identity
- Common Agent ID: `video.delivery_packager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain delivery packager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.delivery_packager.v1`.
- Prompt reference: `video.prompt.delivery_packager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.delivery_packager/agent_spec.json)
...
- Relationship rationale: Human-reviewed mapping of video.delivery_packager to generic source agent(s) video.distributor for self-contained SPEC distillation. Distinct relationship for common role `video.delivery_packager` (inventory order). Reviewed relationship unique to common agent `video.delivery_packager`.

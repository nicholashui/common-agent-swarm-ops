...
## Identity
- Common Agent ID: `video.quality_controller`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain quality controller outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.quality_controller.v1`.
- Prompt reference: `video.prompt.quality_controller.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.quality_controller/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.critic`, `video.judge`.
- Relationship rationale: Human-reviewed mapping of video.quality_controller to generic source agent(s) video.critic, video.judge for self-contained SPEC distillation. Distinct relationship for common role `video.quality_controller` (inventory order). Reviewed relationship unique to common agent `video.quality_controller`.

...
## Identity
- Common Agent ID: `video.graph_topology_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain graph topology designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.orchestrator)

Runs CrewAI/AutoGen/LangGraph DAG; retries, timeouts, fan-out/fan-in

### Distilled quality (video.orchestrator)
...
- Local rubric reference: `video.rubric.graph_topology_designer.v1`.
- Prompt reference: `video.prompt.graph_topology_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.graph_topology_designer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.orchestrator`, `video.router`.
- Relationship rationale: Human-reviewed mapping of video.graph_topology_designer to generic source agent(s) video.orchestrator, video.router for self-contained SPEC distillation. Distinct relationship for common role `video.graph_topology_designer` (inventory order). Reviewed relationship unique to common agent `video.graph_topology_designer`.

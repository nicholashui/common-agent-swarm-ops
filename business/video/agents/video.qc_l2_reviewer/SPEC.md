...
## Identity
- Common Agent ID: `video.qc_l2_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain qc l2 reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.qc_l2_reviewer.v1`.
- Prompt reference: `video.prompt.qc_l2_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.qc_l2_reviewer/agent_spec.json)
...
- Relationship rationale: Human-reviewed mapping of video.qc_l2_reviewer to generic source agent(s) video.critic, video.judge for self-contained SPEC distillation. Distinct relationship for common role `video.qc_l2_reviewer` (inventory order). Reviewed relationship unique to common agent `video.qc_l2_reviewer`.

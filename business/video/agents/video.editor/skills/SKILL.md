---
name: video-editor
description: Role harness for video.editor — Assemble cut; pacing; coverage selection
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.editor` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.editor.v1` from `../prompts/`.
2. Load rubric `video.rubric.editor.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.editor` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.editor/golden.json`

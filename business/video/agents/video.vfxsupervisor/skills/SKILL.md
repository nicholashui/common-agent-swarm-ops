---
name: video-vfxsupervisor
description: Role harness for video.vfxsupervisor — Plans + supervises VFX pipeline
version: 1.0.0
agent_id: video.vfxsupervisor
---

# Skill — `video.vfxsupervisor`

## When to use
Load this skill when the host routes a task to `video.vfxsupervisor` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.vfxsupervisor.v1` from `../prompts/`.
2. Load rubric `video.rubric.vfxsupervisor.v1` from `../rubrics/`.
3. Ground only on `../sources/` and host memory namespaces.
4. Execute architecture patterns: LLM-as-Judge, Agentic Graph, Agent Skills.
...
- **Entry:** pack agent `video.vfxsupervisor` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.vfxsupervisor/golden.json`

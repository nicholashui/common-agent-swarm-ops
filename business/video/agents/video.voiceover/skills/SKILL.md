...
description: Role harness for video.voiceover — Narration, character VO, ad reads
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.voiceover` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.voiceover.v1` from `../prompts/`.
2. Load rubric `video.rubric.voiceover.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.voiceover` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.voiceover/golden.json`

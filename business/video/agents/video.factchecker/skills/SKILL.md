---
name: video-factchecker
description: Role harness for video.factchecker — Source-grade every claim
version: 1.0.0
agent_id: video.factchecker
---

# Skill — `video.factchecker`

## When to use
Load this skill when the host routes a task to `video.factchecker` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.factchecker.v1` from `../prompts/`.
2. Load rubric `video.rubric.factchecker.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.factchecker` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.factchecker/golden.json`

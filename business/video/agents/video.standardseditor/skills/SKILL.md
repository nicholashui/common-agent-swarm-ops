---
name: video-standardseditor
description: Role harness for video.standardseditor — Enforces editorial standards, sourcing discipline, and corrections policy
version: 1.0.0
agent_id: video.standardseditor
---

# Skill — `video.standardseditor`

## When to use
Load this skill when the host routes a task to `video.standardseditor` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.standardseditor.v1` from `../prompts/`.
2. Load rubric `video.rubric.standardseditor.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.standardseditor` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.standardseditor/golden.json`
- Must not require live network for L1 pass when tools are mocked.

<!-- RETHINK_100:start -->
## RETHINK_100 harness notes

Source: `business/video/corpus/study/ui/RETHINK_100_IMPROVEMENTS.md` (applied ids: 14, 15, 20, 21, 26, 30, 31, 35, 37, 38, 40, 43, 48, 49, 50, 59, 63, 77, 86, 87…).

...
## Identity
- Common Agent ID: `video.talent_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain talent coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.talent)

AI-rendered performance

### Distilled quality (video.talent)

Emotion-target match; charisma score (audience proxy)

### Distilled responsibility (video.casting)

Voice + likeness selection; audition simulation

### Distilled quality (video.casting)
...
- Local rubric reference: `video.rubric.talent_coordinator.v1`.
- Prompt reference: `video.prompt.talent_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.talent_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.casting`, `video.talent`.
- Relationship rationale: Human-reviewed mapping of video.talent_coordinator to generic source agent(s) video.talent, video.casting for self-contained SPEC distillation. Distinct relationship for common role `video.talent_coordinator` (inventory order). Reviewed relationship unique to common agent `video.talent_coordinator`.

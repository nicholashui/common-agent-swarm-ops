...
## Identity
- Common Agent ID: `video.compositing_artist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain compositing artist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.vfxsupervisor)

Plans + supervises VFX pipeline

### Distilled quality (video.vfxsupervisor)
...
- Local rubric reference: `video.rubric.compositing_artist.v1`.
- Prompt reference: `video.prompt.compositing_artist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.compositing_artist/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.editor`, `video.vfxsupervisor`.
- Relationship rationale: Human-reviewed mapping of video.compositing_artist to generic source agent(s) video.vfxsupervisor, video.editor for self-contained SPEC distillation. Distinct relationship for common role `video.compositing_artist` (inventory order). Reviewed relationship unique to common agent `video.compositing_artist`.

...
## Identity
- Common Agent ID: `video.legal_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain legal reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.legal_reviewer.v1`.
- Prompt reference: `video.prompt.legal_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.legal_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.legal`.
- Relationship rationale: Human-reviewed mapping of video.legal_reviewer to generic source agent(s) video.legal for self-contained SPEC distillation. Distinct relationship for common role `video.legal_reviewer` (inventory order). Reviewed relationship unique to common agent `video.legal_reviewer`.

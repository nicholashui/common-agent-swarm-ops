---
name: video-ugccreator
description: Role harness for video.ugccreator — Authentic-feel ads in creator voice
version: 1.0.0
agent_id: video.ugccreator
---

# Skill — `video.ugccreator`

## When to use
Load this skill when the host routes a task to `video.ugccreator` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.ugccreator.v1` from `../prompts/`.
2. Load rubric `video.rubric.ugccreator.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.ugccreator` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.ugccreator/golden.json`

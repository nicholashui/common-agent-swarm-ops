...
## Identity
- Common Agent ID: `video.latency_analyst`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain latency analyst outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.latencyoptimizer)

Parallelization, caching, speculative decoding, batching

### Distilled quality (video.latencyoptimizer)
...
- Local rubric reference: `video.rubric.latency_analyst.v1`.
- Prompt reference: `video.prompt.latency_analyst.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.latency_analyst/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.latencyoptimizer`.
- Relationship rationale: Human-reviewed mapping of video.latency_analyst to generic source agent(s) video.latencyoptimizer for self-contained SPEC distillation. Distinct relationship for common role `video.latency_analyst` (inventory order). Reviewed relationship unique to common agent `video.latency_analyst`.

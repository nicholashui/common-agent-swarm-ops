...
## Identity
- Common Agent ID: `video.localization_specialist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain localization specialist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.localization_specialist.v1`.
- Prompt reference: `video.prompt.localization_specialist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.localization_specialist/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.localizationqa`.
- Relationship rationale: Human-reviewed mapping of video.localization_specialist to generic source agent(s) video.localizationqa for self-contained SPEC distillation. Distinct relationship for common role `video.localization_specialist` (inventory order). Reviewed relationship unique to common agent `video.localization_specialist`.

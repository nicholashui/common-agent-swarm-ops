...
## Identity
- Common Agent ID: `video.storyboard_artist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain storyboard artist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.storyboard)

Script → shot panels

### Distilled quality (video.storyboard)
...
- Local rubric reference: `video.rubric.storyboard_artist.v1`.
- Prompt reference: `video.prompt.storyboard_artist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.storyboard_artist/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.storyboard`.
- Relationship rationale: Human-reviewed mapping of video.storyboard_artist to generic source agent(s) video.storyboard for self-contained SPEC distillation. Distinct relationship for common role `video.storyboard_artist` (inventory order). Reviewed relationship unique to common agent `video.storyboard_artist`.

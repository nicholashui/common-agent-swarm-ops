...
## Identity
- Common Agent ID: `video.sound_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain sound designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.sounddesign)

Ambience, foley, SFX

### Distilled quality (video.sounddesign)
...
- Local rubric reference: `video.rubric.sound_designer.v1`.
- Prompt reference: `video.prompt.sound_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.sound_designer/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.sounddesign`.
- Relationship rationale: Human-reviewed mapping of video.sound_designer to generic source agent(s) video.sounddesign for self-contained SPEC distillation. Distinct relationship for common role `video.sound_designer` (inventory order). Reviewed relationship unique to common agent `video.sound_designer`.

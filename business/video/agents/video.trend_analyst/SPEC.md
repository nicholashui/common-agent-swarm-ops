...
## Identity
- Common Agent ID: `video.trend_analyst`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain trend analyst outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.trendintelligence)

Detects emerging memes, sounds, formats

### Distilled quality (video.trendintelligence)
...
- Local rubric reference: `video.rubric.trend_analyst.v1`.
- Prompt reference: `video.prompt.trend_analyst.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.trend_analyst/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.trendintelligence`.
- Relationship rationale: Human-reviewed mapping of video.trend_analyst to generic source agent(s) video.trendintelligence for self-contained SPEC distillation. Distinct relationship for common role `video.trend_analyst` (inventory order). Reviewed relationship unique to common agent `video.trend_analyst`.

...
## Identity
- Common Agent ID: `video.vfx_supervisor`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain vfx supervisor outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.vfx_supervisor.v1`.
- Prompt reference: `video.prompt.vfx_supervisor.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.vfx_supervisor/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.vfxsupervisor`.
- Relationship rationale: Human-reviewed mapping of video.vfx_supervisor to generic source agent(s) video.vfxsupervisor for self-contained SPEC distillation. Distinct relationship for common role `video.vfx_supervisor` (inventory order). Reviewed relationship unique to common agent `video.vfx_supervisor`.

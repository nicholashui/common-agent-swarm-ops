...
## Identity
- Common Agent ID: `video.caption_specialist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain caption specialist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
### Distilled responsibility (video.localizationqa)

Translation + cultural fit

### Distilled quality (video.localizationqa)
...
- Local rubric reference: `video.rubric.caption_specialist.v1`.
- Prompt reference: `video.prompt.caption_specialist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.caption_specialist/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.accessibility`, `video.localizationqa`.
- Relationship rationale: Human-reviewed mapping of video.caption_specialist to generic source agent(s) video.accessibility, video.localizationqa for self-contained SPEC distillation. Distinct relationship for common role `video.caption_specialist` (inventory order). Reviewed relationship unique to common agent `video.caption_specialist`.

...
## Identity
- Common Agent ID: `video.risk_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain risk manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.risk_manager.v1`.
- Prompt reference: `video.prompt.risk_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.risk_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.ethics`, `video.gatekeeper`.
- Relationship rationale: Human-reviewed mapping of video.risk_manager to generic source agent(s) video.ethics, video.gatekeeper for self-contained SPEC distillation. Distinct relationship for common role `video.risk_manager` (inventory order). Reviewed relationship unique to common agent `video.risk_manager`.

...
## Identity
- Common Agent ID: `video.refine_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain refine coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.promptoptimizer)

Auto-improves prompts via OPRO/APE/DSPy/Promptbreeder

### Distilled quality (video.promptoptimizer)

Score uplift per iteration; convergence speed

### Distilled responsibility (video.critic)

Simulates reviewer, press, or jury interpretation

### Distilled quality (video.critic)
...
- Local rubric reference: `video.rubric.refine_coordinator.v1`.
- Prompt reference: `video.prompt.refine_coordinator.v1`; references are inert local contract identifiers.
- Critique edges: `{"inputs":["video.judge_agent"],"outputs":["video.critique_coordinator"]}`.
...
```json
{"agent_id":"video.refine_coordinator","allowed_tools":[],"budget_policy":{"max_input_tokens":2048,"max_output_tokens":1024,"max_tool_requests":0},"critique_edges":{"inputs":["video.judge_agent"],"outputs":["video.critique_coordinator"]},"max_refinement_count":3,"model_policy":{"model_id":"local-video-config-v1","network_access":false,"provider":"local_deterministic"},"production_activation_requested":false,"prompt_reference":"video.prompt.refine_coordinator.v1","role":"Video Refine Coordinator configuration specialist","rubric_reference":"video.rubric.refine_coordinator.v1","schema_version":"1.0","status":"registered"}
```
...
- [Reviewed local source document](../../agents/video.refine_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.critic`, `video.promptoptimizer`.
- Relationship rationale: Human-reviewed mapping of video.refine_coordinator to generic source agent(s) video.promptoptimizer, video.critic for self-contained SPEC distillation. Distinct relationship for common role `video.refine_coordinator` (inventory order). Reviewed relationship unique to common agent `video.refine_coordinator`.

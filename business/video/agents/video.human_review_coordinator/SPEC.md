...
## Identity
- Common Agent ID: `video.human_review_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain human review coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.human_review_coordinator.v1`.
- Prompt reference: `video.prompt.human_review_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.human_review_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.gatekeeper`, `video.judge`.
- Relationship rationale: Human-reviewed mapping of video.human_review_coordinator to generic source agent(s) video.gatekeeper, video.judge for self-contained SPEC distillation. Distinct relationship for common role `video.human_review_coordinator` (inventory order). Reviewed relationship unique to common agent `video.human_review_coordinator`.

...
## Identity
- Common Agent ID: `video.content_strategist`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain content strategist outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.brandstrategist)

Defines audience-value framing and positioning before script and campaign execution

### Distilled quality (video.brandstrategist)
...
- Local rubric reference: `video.rubric.content_strategist.v1`.
- Prompt reference: `video.prompt.content_strategist.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.content_strategist/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.brandstrategist`, `video.marketing`.
- Relationship rationale: Human-reviewed mapping of video.content_strategist to generic source agent(s) video.brandstrategist, video.marketing for self-contained SPEC distillation. Distinct relationship for common role `video.content_strategist` (inventory order). Reviewed relationship unique to common agent `video.content_strategist`.

...
## Identity
- Common Agent ID: `video.memory_curator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain memory curator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.memory_curator.v1`.
- Prompt reference: `video.prompt.memory_curator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.memory_curator/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.memory`.
- Relationship rationale: Human-reviewed mapping of video.memory_curator to generic source agent(s) video.memory for self-contained SPEC distillation. Distinct relationship for common role `video.memory_curator` (inventory order). Reviewed relationship unique to common agent `video.memory_curator`.

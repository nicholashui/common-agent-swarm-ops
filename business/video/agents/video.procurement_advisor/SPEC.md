...
## Identity
- Common Agent ID: `video.procurement_advisor`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain procurement advisor outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.procurement_advisor.v1`.
- Prompt reference: `video.prompt.procurement_advisor.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.procurement_advisor/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.finance`, `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.procurement_advisor to generic source agent(s) video.finance, video.producer for self-contained SPEC distillation. Distinct relationship for common role `video.procurement_advisor` (inventory order). Reviewed relationship unique to common agent `video.procurement_advisor`.

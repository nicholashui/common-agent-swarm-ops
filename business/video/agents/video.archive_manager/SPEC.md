...
## Identity
- Common Agent ID: `video.archive_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain archive manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.archivemaster)

Produces archive-grade masters and preservation packages

### Distilled quality (video.archivemaster)

Checksum integrity, preservation metadata completeness, archive package validity

### Distilled responsibility (video.archiveproducer)

Packages archival materials and source assets for reuse-heavy or documentary workflows

### Distilled quality (video.archiveproducer)
...
- Local rubric reference: `video.rubric.archive_manager.v1`.
- Prompt reference: `video.prompt.archive_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.archive_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.archivemaster`, `video.archiveproducer`.
- Relationship rationale: Human-reviewed mapping of video.archive_manager to generic source agent(s) video.archivemaster, video.archiveproducer for self-contained SPEC distillation. Distinct relationship for common role `video.archive_manager` (inventory order). Reviewed relationship unique to common agent `video.archive_manager`.

...
## Identity
- Common Agent ID: `video.platform_policy_reviewer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain platform policy reviewer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.mpa)

Prepares rating-related packaging and release-readiness inputs for feature workflows

### Distilled quality (video.mpa)
...
- Local rubric reference: `video.rubric.platform_policy_reviewer.v1`.
- Prompt reference: `video.prompt.platform_policy_reviewer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.platform_policy_reviewer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.mpa`, `video.trustsafety`.
- Relationship rationale: Human-reviewed mapping of video.platform_policy_reviewer to generic source agent(s) video.mpa, video.trustsafety for self-contained SPEC distillation. Distinct relationship for common role `video.platform_policy_reviewer` (inventory order). Reviewed relationship unique to common agent `video.platform_policy_reviewer`.

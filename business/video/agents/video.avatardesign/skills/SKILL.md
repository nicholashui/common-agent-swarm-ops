---
name: video-avatardesign
description: Role harness for video.avatardesign — Synthetic-presenter identity
version: 1.0.0
agent_id: video.avatardesign
---

# Skill — `video.avatardesign`

## When to use
Load this skill when the host routes a task to `video.avatardesign` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.avatardesign.v1` from `../prompts/`.
2. Load rubric `video.rubric.avatardesign.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.avatardesign` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.avatardesign/golden.json`

---
name: video-roasoptimizer
description: Role harness for video.roasoptimizer — Optimizes ad creatives for performance
version: 1.0.0
agent_id: video.roasoptimizer
---

# Skill — `video.roasoptimizer`

## When to use
Load this skill when the host routes a task to `video.roasoptimizer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.roasoptimizer.v1` from `../prompts/`.
2. Load rubric `video.rubric.roasoptimizer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.roasoptimizer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.roasoptimizer/golden.json`

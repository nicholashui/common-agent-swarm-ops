---
name: video-styletransfer
description: Role harness for video.styletransfer — Applies named aesthetic consistently across shots
version: 1.0.0
agent_id: video.styletransfer
---

# Skill — `video.styletransfer`

## When to use
Load this skill when the host routes a task to `video.styletransfer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.styletransfer.v1` from `../prompts/`.
2. Load rubric `video.rubric.styletransfer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.styletransfer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.styletransfer/golden.json`

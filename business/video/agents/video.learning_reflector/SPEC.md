...
## Identity
- Common Agent ID: `video.learning_reflector`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain learning reflector outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.learning_reflector.v1`.
- Prompt reference: `video.prompt.learning_reflector.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.learning_reflector/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.evaluationharness`, `video.memory`.
- Relationship rationale: Human-reviewed mapping of video.learning_reflector to generic source agent(s) video.memory, video.evaluationharness for self-contained SPEC distillation. Distinct relationship for common role `video.learning_reflector` (inventory order). Reviewed relationship unique to common agent `video.learning_reflector`.

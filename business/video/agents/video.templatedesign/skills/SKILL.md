---
name: video-templatedesign
description: Role harness for video.templatedesign — Designs reusable and safe personalization templates
version: 1.0.0
agent_id: video.templatedesign
---

# Skill — `video.templatedesign`

## When to use
Load this skill when the host routes a task to `video.templatedesign` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.templatedesign.v1` from `../prompts/`.
2. Load rubric `video.rubric.templatedesign.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.templatedesign` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.templatedesign/golden.json`

...
## Identity
- Common Agent ID: `video.campaign_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain campaign manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.marketing)

Packages content for launch, promotions, and release sequencing

### Distilled quality (video.marketing)

Metadata completeness, asset readiness, launch sequencing accuracy

### Distilled responsibility (video.performancemarketer)

Optimize ads for ROAS

### Distilled quality (video.performancemarketer)
...
- Local rubric reference: `video.rubric.campaign_manager.v1`.
- Prompt reference: `video.prompt.campaign_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.campaign_manager/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.marketing`, `video.performancemarketer`.
- Relationship rationale: Human-reviewed mapping of video.campaign_manager to generic source agent(s) video.marketing, video.performancemarketer for self-contained SPEC distillation. Distinct relationship for common role `video.campaign_manager` (inventory order). Reviewed relationship unique to common agent `video.campaign_manager`.

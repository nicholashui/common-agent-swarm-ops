...
## Identity
- Common Agent ID: `video.shot_planner`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain shot planner outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.cameraoperator)

Executes framing / focus / move per DoP intent

### Distilled quality (video.cameraoperator)
...
- Local rubric reference: `video.rubric.shot_planner.v1`.
- Prompt reference: `video.prompt.shot_planner.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.shot_planner/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.cameraoperator`, `video.director`.
- Relationship rationale: Human-reviewed mapping of video.shot_planner to generic source agent(s) video.cameraoperator, video.director for self-contained SPEC distillation. Distinct relationship for common role `video.shot_planner` (inventory order). Reviewed relationship unique to common agent `video.shot_planner`.

---
name: video-crm
description: Role harness for video.crm — Delivers audience-targeted or trigger-based campaigns through CRM systems
version: 1.0.0
agent_id: video.crm
---
...
## When to use
Load this skill when the host routes a task to `video.crm` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.crm.v1` from `../prompts/`.
2. Load rubric `video.rubric.crm.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.crm` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.crm/golden.json`

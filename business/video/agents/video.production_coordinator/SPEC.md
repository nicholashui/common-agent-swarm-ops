...
## Identity
- Common Agent ID: `video.production_coordinator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain production coordinator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.production_coordinator.v1`.
- Prompt reference: `video.prompt.production_coordinator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.production_coordinator/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.producer`.
- Relationship rationale: Human-reviewed mapping of video.production_coordinator to generic source agent(s) video.producer for self-contained SPEC distillation. Distinct relationship for common role `video.production_coordinator` (inventory order). Reviewed relationship unique to common agent `video.production_coordinator`.

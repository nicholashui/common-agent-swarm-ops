---
name: video-personalizationengineer
description: Role harness for video.personalizationengineer — Variable templates (name/face/voice swap)
version: 1.0.0
agent_id: video.personalizationengineer
---

# Skill — `video.personalizationengineer`

## When to use
Load this skill when the host routes a task to `video.personalizationengineer` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.personalizationengineer.v1` from `../prompts/`.
2. Load rubric `video.rubric.personalizationengineer.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.personalizationengineer` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.personalizationengineer/golden.json`
...
- Agent: `video.personalizationengineer`
<!-- RETHINK_100:end -->

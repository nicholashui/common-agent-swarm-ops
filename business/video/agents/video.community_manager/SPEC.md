...
## Identity
- Common Agent ID: `video.community_manager`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain community manager outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.community)

Captures community response and triages qualitative signals

### Distilled quality (video.community)
...
- Local rubric reference: `video.rubric.community_manager.v1`.
- Prompt reference: `video.prompt.community_manager.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.community_manager/agent_spec.json)
...
## Provenance
- Mapping status: `related`; source-agent IDs: `video.community`.
- Relationship rationale: Human-reviewed mapping of video.community_manager to generic source agent(s) video.community for self-contained SPEC distillation. Distinct relationship for common role `video.community_manager` (inventory order). Reviewed relationship unique to common agent `video.community_manager`.

...
## Identity
- Common Agent ID: `video.orchestrator`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain orchestrator outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.
...
- Local rubric reference: `video.rubric.orchestrator.v1`.
- Prompt reference: `video.prompt.orchestrator.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.orchestrator/agent_spec.json)
...
## Provenance
- Mapping status: `exact`; source-agent IDs: `video.orchestrator`.
- Relationship rationale: Human-reviewed mapping of video.orchestrator to generic source agent(s) video.orchestrator for self-contained SPEC distillation. Distinct relationship for common role `video.orchestrator` (inventory order). Reviewed relationship unique to common agent `video.orchestrator`.

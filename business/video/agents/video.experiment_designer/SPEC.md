...
## Identity
- Common Agent ID: `video.experiment_designer`
- Status: `registered`
- Maturity: `L0`
- Pack version: `0.2.0`

## Responsibility
Owns the video-domain experiment designer outcome by producing a reviewable video artifact, applying the approved pack rubric, recording acceptance criteria, and escalating rights, safety, or quality failures before downstream handoff.



### Domain distillation (embedded)

### Distilled responsibility (video.evaluationharness)

Runs benchmarks (VBench, EvalCrafter, MT-Bench, FVD, CLIP-T); posts regressions

### Distilled quality (video.evaluationharness)

Regression precision/recall; alert latency <1h

### Distilled responsibility (video.analyst)

Aggregates business, creative, and technical performance telemetry into decision-ready reports

### Distilled quality (video.analyst)
...
- Local rubric reference: `video.rubric.experiment_designer.v1`.
- Prompt reference: `video.prompt.experiment_designer.v1`; references are inert local contract identifiers.
...
- [Reviewed local source document](../../agents/video.experiment_designer/agent_spec.json)
...
## Provenance
- Mapping status: `composite`; source-agent IDs: `video.analyst`, `video.evaluationharness`.
- Relationship rationale: Human-reviewed mapping of video.experiment_designer to generic source agent(s) video.evaluationharness, video.analyst for self-contained SPEC distillation. Distinct relationship for common role `video.experiment_designer` (inventory order). Reviewed relationship unique to common agent `video.experiment_designer`.

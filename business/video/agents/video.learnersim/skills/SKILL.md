---
name: video-learnersim
description: Role harness for video.learnersim — Simulates learner behavior, confusion points, and assessment performance
version: 1.0.0
agent_id: video.learnersim
---

# Skill — `video.learnersim`

## When to use
Load this skill when the host routes a task to `video.learnersim` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.learnersim.v1` from `../prompts/`.
2. Load rubric `video.rubric.learnersim.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.learnersim` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.learnersim/golden.json`

---
name: video-finance
description: Role harness for video.finance — Accurate market / earnings / token facts
version: 1.0.0
...
## When to use
Load this skill when the host routes a task to `video.finance` or when composing a swarm step that requires this craft role.

## Instructions
1. Load prompt `video.prompt.finance.v1` from `../prompts/`.
2. Load rubric `video.rubric.finance.v1` from `../rubrics/`.
...
- **Entry:** pack agent `video.finance` via host agent runner
...
## Tests
- Offline golden: `business/video/evals/agents/video.finance/golden.json`
